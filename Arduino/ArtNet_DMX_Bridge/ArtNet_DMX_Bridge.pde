...
void artnet_dmx() {
  ...
  Serial.write(dmx.dmx_universe,dmx.length,&dmx_buffer_sent);
...
void parse_artnet_fragment(uint8_t *data,uint8_t length,uint8_t fragment_no,uint8_t *ip, uint16_t port) {
  ...
  DEBUG("PARSE ENV POSITION: " ,pe.position);
  DEBUG("PARSE FRAGMENT NO: ", fragmen_no);
...
void read_datagram(void (*fragment_callback)(uint8_t *data_buf,uint8_t len,uint8_t fragmentNo,uint8_t *ip, uint16_t port)) {
  ...
  if(available > 0) {
      ...
      READ_SHORT(&recv_len,hdr_buf+6);
      if(recv_len <= 0) goto SKIP;    // Early exit
      ...
      for(uint8_t i=0;i <= fragments;i++) {
          ...
          fragment_callback(fragment_buf,fragment_recv_len,i,src_ip,port);
      }
