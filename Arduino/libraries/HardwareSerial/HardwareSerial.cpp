...
uint8_t *volatile external_buffer;
volatile uint8_t tx_internal_buffer = 1;
void(*volatile external_completion_routine)();
...
uint8_t HardwareSerial::write(uint8_t *data,uint16_t length,void(*completion_routine)()) {
    ...
    // Advance the head
    tx_buffer.head = length-1;  // This might be wrong (it is determined empirically from the single use of this function ...)
	tx_buffer.tail = 0;
 ...
 * [-------------------------------------]
 *  ^HEAD - Advanced by WRITE
 *  ^TAIL - Advanced by EMIT
 */
#if defined(__AVR_ATmega8__)
ISR(SIG_USART_DATA)
#else
...
#endif
{
	if(tx_internal_buffer) {
		...
		if (tx_buffer.head == tail) {
			...
			tx_buffer.tail = (tail + 1) & TX_BUFFER_MASK;
	#if defined(__AVR_ATmega8__)
			UDR = tx_buffer.buffer[tail];
	#else
			UDR0 = tx_buffer.buffer[tail];
	#endif
		}
    } else {
		// External buffer is non-circular
		if (tx_buffer.head == tx_buffer.tail) {
			...
			UCSR0B &= ~(1<<UDRIE0);
#endif
			// And call completion routine
			external_completion_routine();
			tx_internal_buffer = 1;
		} else {			
			tx_buffer.tail++;
#if defined(__AVR_ATmega8__)
			UDR = external_buffer[tx_buffer.tail];
#else
			UDR0 = external_buffer[tx_buffer.tail];
#endif
		}
	}
...
#if defined(__AVR_ATmega1280__)
HardwareSerial Serial1(&rx_buffer1,&tx_buffer, &UBRR1H, &UBRR1L, &UCSR1A, &UCSR1B, &UDR1, RXEN1, TXEN1, RXCIE1, UDRE1, U2X1);
HardwareSerial Serial2(&rx_buffer2,&tx_buffer, &UBRR2H, &UBRR2L, &UCSR2A, &UCSR2B, &UDR2, RXEN2, TXEN2, RXCIE2, UDRE2, U2X2);
HardwareSerial Serial3(&rx_buffer3,&tx_buffer, &UBRR3H, &UBRR3L, &UCSR3A, &UCSR3B, &UDR3, RXEN3, TXEN3, RXCIE3, UDRE3, U2X3);
#endif
